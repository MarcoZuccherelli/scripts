...
# $1 - File con la lista delle LPAR da controllare
...
#  fi
  ...
  if [[ ${desired_proc_units} == 0.0 ]]; then
    desired_proc_units=$(echo $profile | cut -f "11" -d ":")
  fi
#  echo "desired_proc_units orig --> ${desired_proc_units}"
  desired_proc_units=$(printf "%s\n" "scale=1; ${desired_proc_units}/4" | bc)
#  echo "desired_proc_units calcolato --> ${desired_proc_units}"
  if [[ ${desired_proc_units} < "0.5" ]]; then
    desired_proc_units=0.5
  fi

  #Maximum processing unit
  max_proc_units=$(echo $profile | cut -f "9" -d ":")
  if [[ ${max_proc_units} == 0.0 ]]; then
    max_proc_units=$(echo $profile | cut -f "12" -d ":")
  fi
    ...
    for net_dr in ${networks_dr}; do
      ...
      if [[ ${vlan} == ${vlan_dr} ]]; then
        if [[ ${slot} != ${slot_dr} ]]; then
          echo "vlan = ${vlan} slot = k${slot}k -> vlan_dr = ${vlan_dr} slot_dr = k${slot_dr}k"
        #else
        ...
        if [[ ${slot} == ${slot_dr} ]]; then
          echo "vlan = ${vlan} slot = z${slot}z -> vlan_dr = ${vlan_dr} slot_dr = z${slot_dr}z"
          check_vlan="1"
        fi
      fi
    done
